...
int getFCBFromPath(const char * path,myfcb* returnFCB){
  ...
  while (token != NULL) {
    ...
    result = unqlite_kv_fetch(pDb, dirValue[index].referencedFCB, KEY_SIZE, &value,
                                  &nBytes);
...
int myfs_mkdir(const char *path, mode_t mode){
  ...
  mode |= S_IFDIR;
	...
	// mode_t newMode |= S_IFDIR|S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH;

	//  |= ;
	//TODO Set the Parents up with the new UUID
myfcb parent;
unqlite_int64 nBytes;
  int parent = getParentFCB(path,&parent);
  if (parent.size ==0){
    unqlite_kv_fetch(pDb, value.file_data_id, KEY_SIZE, dirValue,
                                 &nBytes);
  } else {
  ...
  uuid_t uid;
  uuid_generate(uid)
	int rc = unqlite_kv_store(pDb,uid,ROOT_OBJECT_KEY_SIZE,&newFCB,sizeof(myfcb));
...
//TODO
  ...
  if (!S_ISDIR(test.mode)) return -1;
  int res = getFCBFromPath(path,&test);
  if (res < 0){
    return -ENOENT;
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    result = unqlite_kv_fetch(pDb, dirents[i].referencedFCB, KEY_SIZE, &loop,
                                  &nBytes);
    if (result < 0 || nBytes != sizeof(myfcb)) return -ENOENT;
